...
export default function NewsSlider({ newsList }: NewsSliderProps): JSX.Element {
  ...
  return (
    <div className={clsx('container', sliderStyles['slider'])}>
      ...
      >
        {newsList.map((news) => {
          return (
            <SwiperSlide className={sliderStyles['slider__item']} key={news.id}>
              <Card data={news} />
            </SwiperSlide>
          )
        })}
      ...
      <Link className="button button--accent" href="/news">
        Посмотреть ещё
      </Link>
    </div>
  )
