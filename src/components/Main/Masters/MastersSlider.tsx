...
type MastersSliderProps = {
  mastersList: CardItem[]
}

export default function MastersSlider({ mastersList }: MastersSliderProps): JSX.Element {
  ...
  return (
    <div className={clsx('container', sliderStyles['slider'])} data-animate="fade-up">
      ...
      >
        {mastersList.map((master: CardItem) => {
          return (
            <SwiperSlide className={sliderStyles['slider__item']} key={master.id}>
              <Card data={master} />
            </SwiperSlide>
          )
        })}
