...
export default function Reviews(): JSX.Element {
    return (
        <section className={clsx('section', reviewsStyles['reviews'])}>
            <div className="container">
                <h2 className="section__title">
                    Отзывы
                </h2>

                <ul className={reviewsStyles['reviews__list']}>
                    {
                        reviewsList.map((review) => {
                            return (
                                <li className={reviewsStyles['reviews__item']} key={review.id}>
                                    <svg className={reviewsStyles['reviews__item-decor']} xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 24 24">
                                        <g clip-path="url(#clip0_3796_252)">
                                    ...
                                    <div className={reviewsStyles['reviews__item-text']}>
                                        {review.text}
                                    </div>

                                    <div className={reviewsStyles['reviews__item-info']}>
                                        <time className={reviewsStyles['reviews__item-date']} dateTime={review.date}>
                                            {convertDateToLocale(review.date)}
                                        </time>

                                        <p className={reviewsStyles['reviews__item-name']}>
                                            {review.name}
                                        </p>

                                        <p className={reviewsStyles['reviews__item-rating']}>
                                            <span className={reviewsStyles['reviews__item-rating-value']}>
                                                {review.stars}
                                            </span>

                                            <svg className={reviewsStyles['reviews__item-rating-icon']}
                ...
                <Link className="reviews__button button button--accent" href="/reviews">Посмотреть ещё</Link>
            </div>
        </section>
    );
}