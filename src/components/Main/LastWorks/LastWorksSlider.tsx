...
interface LastWorksSliderProps {
  worksList: CardItem[]
}

export default function LastWorksSlider({ worksList }: LastWorksSliderProps): JSX.Element {
  ...
  return (
    <div className={clsx('container', sliderStyles['slider'])} data-animate="fade-up">
      <div className={sliderStyles['slider__heading']}>
        <h2 className="section__title">Последние работы</h2>
        ...
        <div className={sliderStyles['slider__controls']}>
          <button
            ...
            className={clsx(
              ...
              sliderStyles['slider__navigation-item--prev'],
            )}
          ...
          <button
            ...
            className={clsx(
              sliderStyles['slider__navigation-item'],
              sliderStyles['slider__navigation-item--next'],
            )}
      ...
      >
        {worksList.map((work) => {
          return (
            <SwiperSlide className={sliderStyles['slider__item']} key={work.id}>
              <Card data={work} />
            </SwiperSlide>
          )
        })}
      ...
      <Link className="button button--accent" href="/works">
        Посмотреть ещё
      </Link>
    </div>
  )
