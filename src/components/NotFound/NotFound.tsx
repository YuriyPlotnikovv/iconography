...
export default function NotFound(): JSX.Element {
    ...
    for (let i = 1; i <= 100; i++) {
        stars.push(
            <div className={notfoundStyles['not-found__star']} key={i}></div>
        );
    }
    ...
    return (
        <section className={clsx('section', notfoundStyles['not-found'])}>
            <div className={notfoundStyles['not-found__text-wrapper']}>
                ...
                <Link className="button button--accent" href="/">Перейти на главную</Link>
            </div>

            <div className={notfoundStyles['not-found__window-wrapper']}>
                <div className={notfoundStyles['not-found__window']}>
                    <div className={notfoundStyles['not-found__stars']}>
                        {stars}
                    </div>
                </div>
            </div>
