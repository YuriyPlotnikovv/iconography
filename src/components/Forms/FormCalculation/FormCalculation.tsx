...
export default function FormCalculation(): JSX.Element {
    ...
    const handleOptionKeyDown = (evt: React.KeyboardEvent<HTMLLIElement>, index: number, id: string) => {
    ...
    return (
        <form className={clsx(formStyles['form'], formStyles['form--calculation'])} action="#" autoComplete="off"
              aria-busy={isLoading}>
            {error && <div className={formStyles['form__cell']} style={{color: 'red', marginBottom: 8}}>{error}</div>}
            ...
            <div className={formStyles['form__label']}>
                <div id="calculation-sort" ref={selectRef} className={formStyles['form__select']}>
                    ...
                    >
                        {selectedId ? (prices.find(p => p._id === selectedId)?.size || '—') : 'Размер/категория'}

                        <svg className={formStyles['form__select__icon']} width="10" height="10" viewBox="0 0 10 10"
                    ...
                    >
                        {prices.map((item, index) => (
                            <li
                                ...
                                className={clsx(formStyles['form__select__option'], {[formStyles['form__select__option--selected']]: item._id === selectedId})}
                                data-value={item._id}
            ...
            <div className={formStyles['form__radio-group']}>
                ...
                <div ref={radioSwitchRef} className={formStyles['form__radio-switch']} role="radiogroup"
                    ...
                    <label ref={(el: HTMLLabelElement | null) => {
                        radioLabelRefs.current[0] = el;
                    }} htmlFor="gold-without" className={formStyles['form__radio-option']}>
                        Без золота
                    </label>

                    <label ref={(el: HTMLLabelElement | null) => {
                        radioLabelRefs.current[1] = el;
                    }} htmlFor="gold-all" className={formStyles['form__radio-option']}>
                        Золотой фон и нимб
                    </label>

                    <label ref={(el: HTMLLabelElement | null) => {
                        radioLabelRefs.current[2] = el;
                    }} htmlFor="gold-halo" className={formStyles['form__radio-option']}>
                        Только нимб
                    </label>

                    <span ref={radioIndicatorRef} className={formStyles['form__radio-indicator']} aria-hidden="true"/>
                </div>
            </div>
