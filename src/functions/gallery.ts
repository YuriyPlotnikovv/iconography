 ...
 */
export function prepareGalleryItems(
  items: GalleryTreeItem[],
  parentPath: string = '',
): GalleryItemForClient[] {
  return items.map((item) => {
    ...
    return {
      ...
      fullPath,
      type: item.type,
      imageUrl: getImageUrl(item.image._id, 600, 450),
      imageLargeUrl: getImageUrl(item.image._id, 1920, 1080, { mode: 'bestFit' }),
      imageThumbUrl: getImageUrl(item.image._id, 600, 450),
